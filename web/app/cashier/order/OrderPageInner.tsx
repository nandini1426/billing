...
export default function OrderPageInner() {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await api.post("/orders", {
        ...
        cgst: gstEnabled ? 2.5 : 0,
        sgst: gstEnabled ? 2.5 : 0,
        ...
        <aside style={{ width: 320, background: "#fff", borderLeft: "1px solid #e5e7eb", overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 12, flexShrink: 0 }}>
          ...
          <div style={{ background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 10, padding: 12 }}>
                  ...
                  {suggestions.map((c, i) => (
                    <div key={i}
                      ...
                      <div style={{ color: "#6b7280", fontSize: 12 }}>
                        {c.customer_phone} · {c.order_count} orders
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <input
              ...
              style={{
                width: "100%",
                border: customerPhone.length > 0 && customerPhone.length < 10
                  ? "1px solid #ef4444" : "1px solid #e5e7eb",
                borderRadius: 8, padding: "7px 10px", fontSize: 13, marginBottom: 8
              }}
            />
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <input type="number" min={0} placeholder="₹ off"
              value={discountFixed || ""}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 24, width: 420, maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div id="receipt-content" style={{ fontFamily: "monospace", border: "1px dashed #ccc", borderRadius: 8, padding: 16, fontSize: 12 }}>
              ...
              <div style={{ textAlign: "center", marginBottom: 8 }}>
                ...
                {restaurantSettings.contact && (
                  <div style={{ fontSize: 11, marginTop: 2 }}>
                    Contact No: {restaurantSettings.contact}
                  </div>
                )}
              ...
              {/* Customer name — no phone on receipt */}
              {customerName && (
  ...
  <div style={{ textAlign: "center", marginBottom: 8 }}>
    <div style={{ fontSize: 11, marginBottom: 6 }}>Scan to Pay via UPI app</div>
      ...
      />
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
              <button onClick={() => window.print()}
                style={{ flex: 1, padding: "10px 0", borderRadius: 10, border: "none", background: "#2563eb", color: "#fff", fontWeight: 600, cursor: "pointer", fontSize: 13 }}>
                🖨️ Print Receipt
              </button>
            </div>

            {/* WhatsApp button — only show if customer has phone */}
            {customerPhone && customerPhone.length === 10 && (
              <div style={{ marginTop: 8 }}>
                <button
                  onClick={() => {
                    const message = `*${restaurantSettings.name}*\n` +
                      `${restaurantSettings.address ? restaurantSettings.address + "\n" : ""}` +
                      `${restaurantSettings.contact ? "Contact: " + restaurantSettings.contact + "\n" : ""}` +
                      `${restaurantSettings.gstin ? "GSTIN: " + restaurantSettings.gstin + "\n" : ""}` +
                      `--------------------------------\n` +
                      `Bill No: ${orderNumber || "—"}\n` +
                      `Table: ${mode === "table" ? tableLabel : mode.toUpperCase()}\n` +
                      `Date: ${new Date().toLocaleDateString('en-IN')}\n` +
                      `Time: ${new Date().toLocaleTimeString('en-IN')}\n` +
                      `${customerName ? "Customer: " + customerName + "\n" : ""}` +
                      `--------------------------------\n` +
                      currentOrder.map((item, i) =>
                        `${i + 1}. ${item.name} x${item.quantity} = ₹${Math.round(item.price * item.quantity)}`
                      ).join("\n") +
                      `\n--------------------------------\n` +
                      `Net Total: ₹${bill.subtotal}\n` +
                      (discountPct > 0 || discountFixed > 0 ? `Discount: -₹${Math.round((bill.subtotal - bill.afterDiscount) * 100) / 100}\n` : "") +
                      (gstEnabled ? `CGST (2.5%): ₹${bill.cgst}\nSGST (2.5%): ₹${bill.sgst}\n` : "") +
                      (serviceChargeAmt > 0 ? `Service Charge: ₹${serviceChargeAmt}\n` : "") +
                      `*Gross Amount: ₹${bill.grandTotal}*\n` +
                      `Amount: ${numberToWords(bill.grandTotal)} Rupees only\n` +
                      `--------------------------------\n` +
                      `${restaurantSettings.footer_text}`;

                    const phone = `91${customerPhone}`;
